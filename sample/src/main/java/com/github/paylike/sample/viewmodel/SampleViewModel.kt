...
/**
 * Sample viewModel to handle and store ui logic.
 * Implements [Observer] to be able to listen to the consisting engine.
 */
class SampleViewModel: ViewModel(), Observer {
    /**
     * [paylikeEngine] is set to test mode.
     * You can obtain your own [PaylikeMerchantApiKey] from Paylike
     * and include it in the gradle build files to use it.
     */
     ...
     */
    var uiState = UiStateRepository(
        shouldRenderPayButton = mutableStateOf(isWaitingForInputState(paylikeEngine.currentState)),
        shouldRenderResetButton = mutableStateOf(isSuccessOrErrorState(paylikeEngine.currentState)),
    )
        private set
     ...
     */
    private fun isWaitingForInputState(state: EngineState) =
    ...
    /**
     * Observer update function overload.
     * Sets the necessary state variables for the UI logic.
     */
